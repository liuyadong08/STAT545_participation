...
square <- function(x) {
  y <- x^2
  return(y)
  5 # even though this is the last line of the function, but r won't return 5 since there is a return function above it.
}
square(10)
```
...
Quantile(x, probe) means the max value of the first 90% of x if the probe is 0.9
...
qd2(rnorm(100), probs = c(0.25, 0.9))
```
...
it means whatever goes to the ... position in the function, it will automatically goes to the ... position at qvec
